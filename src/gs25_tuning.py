...
# observation_noise = np.diag([5e-6, 5e-6]) + np.ones((2, 2)) * 1e-9  # 3
...
# T, half_size, noise, obs_noise = best_params
# size = half_size * 2
...
# collection_mtv = [c for c in pairs_all['test'] if str(c).find('SJC') == -1]
# collection_sjc = [c for c in pairs_all['test'] if str(c).find('SJC') != -1]
# # =========================================================================
'''
...
# n_cols = 4
# n_rows = 6
# data, layout = visualize_tile_init(n_rows, n_cols, collection_all)
...
for i, (i_c, c) in enumerate(collection_all[i_route:i_route+1]):
...
# -------------------------------------------
        ...
        data, layout = visualize_tile_append2(
            df_test_ex_0, df_test_ex, df_train_gt, data, layout, i)
        i = i + 1
...
#        pdb.set_trace()
    ...
    data, layout = visualize_tile_append2(
        df_test_ex_0, df_ex, df_train_gt, data, layout, i)
...
sub_prev = pd.read_csv('./submission.csv')
# ix = sub_pred{'poh')
phone = df_test_ex['phone'].unique()[0]
sub_prev.loc[sub_prev['phone'] == phone, ['latDeg', 'lngDeg']]

# print(
#     f"before {sub_prev.loc[sub_prev['phone'] == phone, ['latDeg', 'lngDeg']]}")

if sub_prev[sub_prev['phone'] == phone].shape[0] == df_test_ex.shape[0]:
    sub_prev.loc[sub_prev['phone'] == phone,
                 'latDeg'] = df_test_ex['latDeg'].values
    sub_prev.loc[sub_prev['phone'] == phone,
                 'lngDeg'] = df_test_ex['lngDeg'].values
else:
    print(f'\033[33m SOMETHING IS WRONG\033[0m')

# print(
#     f"after  {sub_prev.loc[sub_prev['phone'] == phone, ['latDeg', 'lngDeg']]}")
...
print(f'\033[1;93msubmission.csv replaced {i_route}\033[0m')
# -------------------------------------------
# HERE4
# -------------------------------------------
# sub.to_csv('submission.csv', index=False)
